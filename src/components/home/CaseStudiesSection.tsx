              ...
              <div className="relative bg-white/80 backdrop-blur-sm overflow-hidden h-full rounded-2xl border border-border group-hover:border-primary/30 shadow-sm group-hover:shadow-xl transition-all duration-500">
                <div className={`h-40 bg-gradient-to-br ${project.gradient} relative overflow-hidden`}>
                  ...
                  <div className="absolute bottom-4 left-6">
                    <span className="px-3 py-1 rounded-full bg-white/90 text-primary text-xs font-semibold border border-primary/20 shadow-sm">
                      ● Live Project
                    </span>
                  </div>
                </div>
                <div className="p-7">
                  <h3 className="font-heading font-bold text-xl mb-3 text-foreground group-hover:text-primary transition-colors">{project.title}</h3>
                  <p className="text-muted-foreground text-sm mb-5 leading-relaxed">{project.description}</p>
                  <div className="flex flex-wrap gap-1.5 mb-6">
                    {project.tech.map((t) => (
                      <span key={t} className="text-xs px-2.5 py-1 bg-primary/8 text-primary rounded-full font-medium">{t}</span>
                    ))}
                  </div>
                  <Button variant="outline" size="sm" asChild className="w-full group/btn border-primary/30 hover:border-primary hover:bg-primary/5 text-foreground hover:text-primary">
